...
import {IconProps,ArrowUpRight } from "phosphor-react-native";
...
interface SumaryTypeStylesProps   {
    type: boolean;
}
export const Summary =  styled.View<SumaryTypeStylesProps>`
    width: '100%';
    border-radius: 10;

    justify-content: center;
    background-color: ${({ theme, type })=> type ? theme.colors.green_300 : theme.colors.red_300 };
...
export const Icon = styled(ArrowUpRight).attrs<IconTypeStylesProps>(({ theme, type} ) => ({
    size:28,
